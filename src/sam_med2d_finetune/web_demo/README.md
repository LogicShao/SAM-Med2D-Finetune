...
```text
src/sam_med2d_finetune/web_demo/
├── app.py
├── config.py
├── README.md
├── services/
...
- 优先复用 `sam_med2d_finetune.inference.volume`、`sam_med2d_finetune.inference.postprocess`、`sam_med2d_finetune.inference.visualize` 和已有 `outputs/`。
...
```bash
export PYTHONPATH=src
python -m uvicorn sam_med2d_finetune.web_demo.app:app --host 127.0.0.1 --port 7860
```
...
1. `python -m sam_med2d_finetune.inference.volume`
2. `sam_med2d_finetune.inference.postprocess` 中的后处理逻辑
3. `python -m sam_med2d_finetune.inference.visualize`
