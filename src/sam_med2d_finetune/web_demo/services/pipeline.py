...
from sam_med2d_finetune.web_demo.services.job_state import (
    build_run_id,
    complete_job,
    create_job,
    fail_job,
...
from sam_med2d_finetune.web_demo.services.logger import RunLogger
from sam_med2d_finetune.web_demo.services.results import encode_result_id, find_viewer_file
...
def _build_summary_steps() -> list[dict[str, Any]]:
    return [
        {
            "key": item["key"],
            "name": item["name"],
...
def _init_pipeline_summary(case_input: dict[str, Any], result_dir: Path) -> dict[str, Any]:
    mode = get_demo_mode(case_input.get("mode_key"))
    return {
        ...
        "finished_at": None,
        "steps": _build_summary_steps(),
        "artifacts": {},
        "error": None,
    }


def _update_summary_step(summary: dict[str, Any], step_key: str, status: str, detail: str) -> None:
    now_text = _now_text()
...
def _build_python_module_command(module_name: str, *args: str) -> list[str]:
...
def run_inference(case_input: dict[str, Any], run_logger: RunLogger | None = None) -> dict[str, Any]:
    ...
    command = _build_python_module_command(
        "sam_med2d_finetune.inference.volume",
        "--case_dir",
...
def run_visualization(result_dir: Path, run_logger: RunLogger | None = None) -> dict[str, Any]:
    ...
    mode = get_demo_mode(mode_key)
    command = _build_python_module_command(
        "sam_med2d_finetune.inference.visualize",
        "--output_dir",
        str(result_dir),
        "--mask_name",
...
def run_full_pipeline(
    ...
    *,
    ...
    try:
        ...
        _update_summary_step(summary, current_stage_key, "running", "正在进行自动分割")
        _write_json(summary_path, summary)
        ...
        _update_summary_step(summary, current_stage_key, "done", "分割结果已生成")
        ...
        _update_summary_step(summary, current_stage_key, "running", "正在进行结果处理")
        _write_json(summary_path, summary)
        ...
        _update_summary_step(summary, current_stage_key, "done", "后处理完成")
        ...
        _update_summary_step(summary, current_stage_key, "running", "正在生成三维结果")
        _write_json(summary_path, summary)
        ...
        _update_summary_step(summary, current_stage_key, "done", "三维模型已生成")
        summary["artifacts"]["viewer"] = visualization_result["viewer_file"]

        result_id = encode_result_id(result_dir)
        duration_text = _format_duration(time.perf_counter() - started)
        ...
        _update_summary_step(summary, current_stage_key, "failed", error_message)
        _write_json(summary_path, summary)

        if run_logger is not None:
            run_logger.error(f"出错信息: {error_message}")
            run_logger.info(f"结果目录: {result_dir}")
            run_logger.info(f"总耗时: {_format_duration(time.perf_counter() - started)}")
        if run_id is not None:
            fail_job(run_id, stage_key=current_stage_key, message=error_message)
        raise


def _run_pipeline_job_in_background(run_id: str, case_input: dict[str, Any], run_logger: RunLogger) -> None:
    try:
...
def start_pipeline_job(case_input: dict[str, Any]) -> dict[str, Any]:
    ...
    thread = threading.Thread(
        target=_run_pipeline_job_in_background,
