...
from sam_med2d_finetune.models.factory import build_multitask_base_model
from sam_med2d_finetune.training.metrics import SegMetrics
from sam_med2d_finetune.training.profiler import GpuUtilizationMonitor, parse_cuda_device_index
from sam_med2d_finetune.utils.cli import str_to_bool
from sam_med2d_finetune.utils.training import FocalDiceloss_IoULoss, get_logger
...
def parse_args():
    ...
    parser.add_argument("--profile_stages", type=str_to_bool, default=False,
                        help="分解记录 image encoder、prompt/decoder/loss 和反传优化器的 CUDA 时间")
...
def train_one_epoch(model, optimizer, train_loader, criterion, args, epoch, scaler):
    ...
    for batched_input in pbar:
        ...
        profile_stages = args.profile_stages and str(args.device).startswith("cuda")
        if profile_stages:
            encoder_start = torch.cuda.Event(enable_timing=True)
            encoder_end = torch.cuda.Event(enable_timing=True)
            prompt_decoder_start = torch.cuda.Event(enable_timing=True)
            prompt_decoder_end = torch.cuda.Event(enable_timing=True)
        ...
        if compute_end is not None:
            ...
            if profile_stages:
                stage_seconds["image_encoder"].append(encoder_start.elapsed_time(encoder_end) / 1000.0)
                stage_seconds["prompt_decoder_loss"].append(
                    prompt_decoder_start.elapsed_time(prompt_decoder_end) / 1000.0
                )
                stage_seconds["backward_optimizer"].append(backward_start.elapsed_time(backward_end) / 1000.0)
        elif profile_stages:
    ...
    runtime_stats = {
        ...
        "mean_image_encoder_seconds": float(np.mean(stage_seconds["image_encoder"])) if stage_seconds["image_encoder"] else None,
        "mean_prompt_decoder_loss_seconds": float(np.mean(stage_seconds["prompt_decoder_loss"])) if stage_seconds["prompt_decoder_loss"] else None,
        "mean_backward_optimizer_seconds": float(np.mean(stage_seconds["backward_optimizer"])
                                               ) if stage_seconds["backward_optimizer"] else None,
    }
    if str(args.device).startswith("cuda"):
...
def main(args):
    ...
    csv_header = ['epoch', 'train_steps', 'train_samples', 'train_elapsed_seconds', 'train_updates_per_second',
                  ...
                  'mean_image_encoder_seconds', 'mean_prompt_decoder_loss_seconds', 'mean_backward_optimizer_seconds',
    ...
    for epoch in range(args.epochs):
        ...
        logger.info(
            "Epoch %d | GPU stages -> image encoder: %s, prompt/decoder/loss: %s, backward/optimizer: %s",
            epoch + 1,
            runtime_stats["mean_image_encoder_seconds"],
            runtime_stats["mean_prompt_decoder_loss_seconds"],
            runtime_stats["mean_backward_optimizer_seconds"],
        )
